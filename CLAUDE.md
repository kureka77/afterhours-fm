...
├── shazam_fallback.py    # Shazam-via-vibra recognition — last-resort fallback
...
- Track-source chain inside `now_playing()`, cheapest and most authoritative first: **ICY -> broadcaster playlist -> Shazam**. Each step only runs if the previous produced no title, so they never run alongside each other.
